...
#include "CqsjPowerfulStateComponent.generated.h"
...
class UCqsjPSBPI : public UInterface
{
	GENERATED_BODY()
};
class CQSJPOWERFULSTATE_API ICqsjPSBPI
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category="Cqsj|PE")
	void OnPowerfulEvent(FGameplayTag EventKey, FPE_Event PE_Event);
	UFUNCTION(BlueprintNativeEvent, BlueprintCallable, Category="Cqsj|PS")
	void OnPowerfulState(FGameplayTag StateKey, FPS_Event PS_Event);
};

UCLASS( ClassGroup=(Custom), meta=(BlueprintSpawnableComponent) )
class CQSJPOWERFULSTATE_API UCqsjPowerfulStateComponent : public UActorComponent
{
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FCqsjPEDlg, FGameplayTag, EventKey, FPE_Event, PE_Event);
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FCqsjPSDlg, FGameplayTag, StateKey, FPS_Event, PS_Event);
	GENERATED_BODY()

public:	
	UCqsjPowerfulStateComponent();
	virtual void GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const override;
	
	UPROPERTY(BlueprintCallable, BlueprintAssignable, Category = "Cqsj|PE")
	FCqsjPEDlg OnPowerfulEvent;
	UPROPERTY(BlueprintCallable, BlueprintAssignable, Category = "Cqsj|PS")
	FCqsjPSDlg OnPowerfulState;

	UPROPERTY(BlueprintReadWrite, EditAnywhere, Replicated, Category="Cqsj|PowerfulState|Settings")
	int MaxStateOperationNum = 2048;
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Replicated, Category="Cqsj|PowerfulState|Debug")
	bool bIsDebug;
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Replicated, Category="Cqsj|PowerfulState|Debug")
	...
	UPROPERTY(BlueprintReadOnly, VisibleAnywhere, Replicated, Category="Cqsj|PowerfulState")
...
//State BEGIN
	...
	UFUNCTION(BlueprintCallable,Category = "Cqsj|PS|Component")
	void SetMaxStateOperationNum(int InMaxStateOperationNum);
	UFUNCTION(Reliable,Server,Category = "Cqsj|PS|Component")
...
//Event BEGIN
	UFUNCTION(BlueprintCallable,Category = "Cqsj|PE|Component")
	void SendEvent(FPE_Event PE_Event);
	UFUNCTION(Reliable,Server,Category = "Cqsj|PE|Component")
	void SendEvent_Server(FPE_Event PE_Event);
	UFUNCTION(Reliable, NetMulticast, Category = "Cqsj|PE|Component")
	void SendEvent_Multicast(FPE_Event PE_Event);
	void SendEvent_Imp(const FPE_Event& PE_Event) const;
//Event END

//Debug Begin
	UFUNCTION(BlueprintCallable,Category = "Cqsj|PS|Component|Debug")
	void ActivePSDebug(FColor InDebugTextColor = FColor::Green, bool InPrintToScreen = true, bool InPrintToLog = true);
	UFUNCTION(Reliable,Server,Category = "Cqsj|PS|Component|Debug")
